...
from witchcraft.utils import __query_paths, build_tuple_type, chainlist, coalesce
...
def query(connection, sql_query):
    if connection.connection is not None and callable(
        getattr(connection.connection, "execute", None)
    ):
...
def batch_fetch_stream(connection, sql_query, batch_size):
    if connection.connection is not None and callable(
        ...
        result_proxy = connection.connection.execution_options(
